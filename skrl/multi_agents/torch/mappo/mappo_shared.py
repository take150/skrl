...
class MAPPO_SHARED(MultiAgent):
    def __init__(
        ...
        self.shared_value = self.models.get("value", None)
        self.checkpoint_modules["value"] = self.shared_value
        ...
        for uid in self.possible_agents:
            ...
            if policy is not None and self.shared_value is not None:
                if policy is self.shared_value:
                    optimizer = torch.optim.Adam(policy.parameters(), lr=self._learning_rate[uid])
                else:
                    optimizer = torch.optim.Adam(
                        itertools.chain(policy.parameters(), self.shared_value.parameters()), lr=self._learning_rate[uid]
                    )
    ...
    def record_transition(
        ...
        if self.memories:
            ...
            for uid in self.possible_agents:
                ...
                # compute values
                with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                    values, _, _ = self.shared_value.act(
                        {"states": self._shared_state_preprocessor[uid](shared_states[uid])}, role="value"
                    )
            ...
            for epoch in range(self._learning_epochs[uid]):
                ...
                for (
                    ...
                    if config.torch.is_distributed:
                        ...
                        if policy is not self.shared_value:
                    ...
                    if self._grad_norm_clip[uid] > 0:
                        ...
                        if policy is self.shared_value:
                            ...
                            nn.utils.clip_grad_norm_(
                                itertools.chain(policy.parameters(), self.shared_value.parameters()), self._grad_norm_clip[uid]
                            )
